...
### Разрешение PowerShell-скриптов в Windows

Перед первым запуском Windows-скрипта разрешите выполнение локальных сценариев для текущего пользователя:
...
Настройка не требует прав администратора и не влияет на других пользователей компьютера. Не используйте `Unrestricted` или область `LocalMachine` для запуска bootstrap-скрипта.

Если в выводе `Get-ExecutionPolicy -List` у `MachinePolicy` или `UserPolicy` указано значение, отличное от `Undefined`, оно имеет приоритет. В этом случае обратитесь к администратору компьютера.
...
- Если PowerShell запрещает запуск Windows-скрипта политикой выполнения, выполните команды из раздела «Разрешение PowerShell-скриптов в Windows». Если они не помогают из-за `MachinePolicy` или `UserPolicy`, обратитесь к администратору компьютера.
